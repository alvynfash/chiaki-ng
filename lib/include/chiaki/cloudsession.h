...
/**
 * JSON boundary for hosts that cannot conveniently mirror the C callback
 * struct. The input contains the named fields from ChiakiCloudProvisionConfig;
 * the returned JSON contains the fields from ChiakiCloudProvisionResult plus
 * `err`. The returned string is heap-owned and must be released with
 * chiaki_cloud_provision_json_free(). The call is blocking and has no progress
 * or cancellation callbacks, so hosts must invoke it off their UI thread.
 */
