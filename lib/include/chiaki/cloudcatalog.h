 ...
 * v9: strips unused storefront source fields from each public contract row.
 * v10: retains genre metadata used by native paged catalog queries. */
...
typedef struct chiaki_cloudcatalog_config_t
{
	...
	const char *scope;      /**< optional: "owned", "psplus", "psnow", or "all"; NULL => "all" */
 ...
 *   }
 *
 * The core API returns "games" in canonical order (owned first, then by name).
 * The JSON host boundary also accepts searchQuery, scope, filter, sort, skip,
 * and take query fields and returns the requested page in Pylux order; clients
 * render that page as-is.
 *
...
/** JSON host boundary for bindings that cannot safely mirror the C structs. */
CHIAKI_EXPORT ChiakiErrorCode chiaki_cloudcatalog_fetch_unified_json(
	const char *config_json, char **result_json);

/** Release JSON returned by chiaki_cloudcatalog_fetch_unified_json(). */
