...
static void copy_contract_field(struct json_object *dst, struct json_object *src,
	const char *key)
{
	struct json_object *value = NULL;
	if(json_object_object_get_ex(src, key, &value))
		json_object_object_add(dst, key, json_object_get(value));
}

// Source responses carry a large amount of storefront metadata that no client
// consumes. Keep only the documented cross-platform contract before writing or
// returning the unified cache; this reduces a typical snapshot from tens of MB
// to the small set of fields needed for cards, ownership and provisioning.
static void compact_catalog_contract(struct json_object *env)
{
	static const char *const fields[] = {
		"productId", "name", "imageUrl", "landscapeImageUrl", "conceptId",
		"category", "serviceType", "platform", "isOwned", "streamServiceType",
		"streamIdentifier", "entitlementId", "storeProductId", "conceptUrl",
		"plusCatalog", "genre", "genres", NULL
	};
	struct json_object *games = cc_json_arr(env, "games");
	struct json_object *compacted = NULL;
	size_t n;

	if(!games)
		return;
	compacted = json_object_new_array();
	if(!compacted)
		return;
	n = json_object_array_length(games);
	for(size_t i = 0; i < n; i++)
	{
		struct json_object *source = json_object_array_get_idx(games, i);
		struct json_object *game = json_object_new_object();
		if(!game)
			continue;
		for(size_t field = 0; fields[field]; field++)
			copy_contract_field(game, source, fields[field]);
		json_object_array_add(compacted, game);
	}

	json_object_object_del(env, "games");
	json_object_object_add(env, "games", compacted);
	json_object_object_del(env, "total");
	json_object_object_add(env, "total",
		json_object_new_int((int)json_object_array_length(compacted)));
}

// Keep the complete unified catalog in the native cache, but return only the
// rows requested by lightweight views. This avoids serializing and copying the
// multi-megabyte all-games payload across FFI just to render the owned library.
static void project_result_scope(struct json_object *env, const char *scope)
{
	if(!env || !scope || strcmp(scope, "owned") != 0)
		return;
	...
	struct json_object *projected = json_object_new_array();
	if(!projected)
		return;
	...
	for(size_t i = 0; i < n; i++)
	{
		...
		if(cc_json_bool(game, "isOwned"))
			json_object_array_add(projected, json_object_get(game));
	}

	json_object_object_del(env, "games");
	json_object_object_add(env, "games", projected);
	json_object_object_del(env, "total");
	json_object_object_add(env, "total",
		json_object_new_int((int)json_object_array_length(projected)));
}
	...
	const ChiakiCloudCatalogConfig *config, ChiakiCloudCatalogResult *out, ChiakiLog *log)
{
	...
	if(!force)
	{
		...
		if(cached)
		{
			...
			if(ver == CHIAKI_CLOUDCATALOG_SCHEMA_VERSION || ver == 8)
			{
				if(ver == 8)
				{
					struct json_object *next_version = json_object_new_int(
				...
				project_result_scope(cached, requested_scope);
	...
	bool apollo_complete = !include_apollo;
	CCNativeResult nr = CC_NATIVE_OK;
	if(include_apollo)
		nr = cc_fetch_psnow_native(log, npsso, &apollo,
	...
	bool browse_complete = !include_imagic;
	char settled[16];
	snprintf(settled, sizeof(settled), "%s", effective_locale);

	struct json_object *v6 = include_imagic && !force
		? cc_cache_read(log, cache_dir, "ps5_cloud_catalog_v6", CC_CACHE_TTL_MS)
		: NULL;
	if(v6)
	{
	...
	const bool apollo_empty = json_object_array_length(apollo) == 0;
	const bool browse_empty = json_object_array_length(browse) == 0;
	const bool required_source_empty =
		(include_apollo && !include_imagic && apollo_empty) ||
		(!include_apollo && include_imagic && browse_empty) ||
		(include_apollo && include_imagic && apollo_empty && browse_empty);
	if(required_source_empty && !auth_error)
	{
	...
	if(total > 0 && !auth_error && apollo_complete && browse_complete && owned_complete)
		cc_cache_write(log, cache_dir, unified_cache_key, env);

	project_result_scope(env, requested_scope);
