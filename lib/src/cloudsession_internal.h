...
/**
 * Probe one datacenter. Cloud services use bounded regional RTT estimates
 * because their Senkusha endpoints reject non-Portal BIG probes; non-cloud
 * callers retain the native Senkusha handshake.
 *
 * @param data_center datacenter name used for regional estimates
 * @param public_ip   datacenter host or IPv4 (resolved here)
 * @param port        datacenter UDP port (typically 40101)
 * @param session_key x-gaikai-session value for native Senkusha probing
 * @param service_type "psnow" (adds the PSN wrapper) or "pscloud"
 * @param out_rtt_us  RTT/estimate in microseconds, or <0 on failure
 * @param out_mtu_in/out_mtu_out  negotiated/default MTU (0 on failure)
 * @return CHIAKI_ERR_SUCCESS when a usable RTT or estimate is available.
 */
