...
static void takion_handle_packet_message_data_ack(ChiakiTakion *takion, uint8_t flags, uint8_t *buf, size_t buf_size)
{
	...
	size_t expected_size = (buf_size >= 0xc ? 0xc : 0xa) + gap_ack_blocks_count * 4 + dup_tsns_count * 4;
	if(buf_size != expected_size)
	{
		CHIAKI_LOGW(takion->log, "Takion received data ack with invalid block counts size=%zx expected=%zx gap_ack_blocks_count=%#x dup_tsns_count=%#x",
